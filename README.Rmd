...
The function returned by `power1` raises its argument to whatever `exponent` is defined as.
...
Due to R's lazy evaluation, when we call `power1`, `my_exponent` doesn't actually have the value of `2` yet.
...
The `my_exponent` promise that was passed in during the definition of `square1a1` resolves to `2` the first time it is needed (when `square1a` is first called). After that initial call, that is the value used in `square1a` forever.
...
We don't really need to use `force`. 
Anything that forces evaluation would work, but `force` makes it obvious why we're doing it.
